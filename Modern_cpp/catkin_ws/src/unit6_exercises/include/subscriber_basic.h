...
template <typename typeMSG> class MagicSubscriber {
public:
  MagicSubscriber(ros::NodeHandle &ros_node, const string subscriber_topic) {
    ...
    image_sub_ = this->m_ros_node_object->subscribe(
        this->m_subscriber_topic, 1,  &MagicSubscriber::CallbackToTopic<typeMSG>, this);
  }
  ...
  template <typename CallBackROSMessageType> void CallbackToTopic(const typename typeMSG::ConstPtr &msg) {
